...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    """
    return max(tweets, key=lambda t: t.retweets / t.time if t.time > 0 else 0)
...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    for tweet in tweets:
        ...
        for tag in hashtags:
            hashtag_popularity[tag] += tweet.retweets

    return sorted(hashtag_popularity, key=lambda h: (-hashtag_popularity[h], h))
